...
uint16_t Get_Mean(uint16_t new) {
	uint16_t old = sample[headIndex];
	sample[headIndex] = new;
	headIndex = (headIndex + 1) % 6;
	sum = sum - old + new;
	uint16_t mean = sum/6;
	//sprintf(String, "sum: %u , mean: %u \n", sum, mean);
	...
	while (1)
	{
		...
		if (!(PINC & (1 << PINC0))) {
			...
			if (game_state == 0) {
				...
				if ((game_state == 1) || (game_state == 11) || (game_state == 21)) {
					...
					if ((game_state == 11) && (in_boundary(x_screen, y_screen, 0, 40 + 63 * 128)) ) {
						if (player_y < array[0] / 128 + 5) {
							if (!(array[0] / 128 <= 1)) {
								array[0] = player_y* 128;
							}
